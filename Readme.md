...
- **ドラッグ＆ドロップ**  
ウィンドウにPDFを落とすと処理が始まる。複数ファイルをまとめて落とせる。Outlook(Classic)のメール添付のように、ディスク上に実体を持たない添付（仮想ファイル）も直接落とせる。
- **貼り付け（Ctrl+V）**  
Outlookで添付ファイルをコピーし、ウィンドウ上で `Ctrl+V`（または「貼り付け」ボタン）を押すと、同じ処理を行う。ドラッグ＆ドロップが効かない場面の受け口。
- **ファイル選択ダイアログ**  
...
3. PDFをウィンドウにドラッグ＆ドロップする（Outlookの添付をコピーして `Ctrl+V`、または「ファイルを選択...」ボタンから選んでもよい）
